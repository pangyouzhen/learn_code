...
class ESIM:
    def __init__(self, is_training, init_embedding_up, seq_length, class_num, vocabulary_size, embedding_size,
        ...
        with tf.name_scope("Input_encoding"):
            a_bar = self.biLSTMBlock(self.text_a_embedding, hidden_num, "Input_Encoding/cnn")
            b_bar = self.biLSTMBlock(self.text_b_embedding, hidden_num, "Input_Encoding/cnn")
        ...
        with tf.name_scope("inference_composition"):
            v_a = self.biLSTMBlock(self.m_a, hidden_num, "inference_composition/cnn")
            v_b = self.biLSTMBlock(self.m_b, hidden_num, "inference_composition/cnn")
    ...
    def biLSTMBlock(self, inputs, num_units, scope):
        with tf.variable_scope(scope, reuse=tf.AUTO_REUSE):
            cell_fw = tf.nn.rnn_cell.BasicLSTMCell(num_units)
            cell_bw = tf.nn.rnn_cell.BasicLSTMCell(num_units)
            (a_outputs, a_outputs_state) = tf.nn.bidirectional_dynamic_rnn(cell_fw, cell_bw,
